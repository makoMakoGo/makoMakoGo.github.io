---
title: Claude Code forked subagent 机制说明
date: 2026-04-30
...
- `CLAUDE_CODE_FORK_SUBAGENT=1` 启用的是 **forked subagent**：子 agent 从当前主会话分叉，继承已有上下文。
- forked subagent 默认后台运行；如果设置 `CLAUDE_CODE_DISABLE_BACKGROUND_TASKS=1`，则以前台同步方式运行。
- fork 适合强依赖当前上下文、重新 briefing 成本高的并行任务；named subagent 更适合固定角色、独立 system prompt 和 fresh context。
- `/fork` 在该开关下用于启动 forked subagent；如果需要原来的分支对话，应显式使用 `/branch`。
