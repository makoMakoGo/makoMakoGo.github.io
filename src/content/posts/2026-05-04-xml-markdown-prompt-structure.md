---
title: Markdown Heading 与 XML Tag 在全局提示词中的混用原则
date: 2026-05-04
...
Markdown heading 和 XML-like tags 可以混用，而且在长期维护的全局提示词里适合混用。

推荐分工是：Markdown heading 负责文档骨架和人类维护，XML tag 负责语义边界、示例边界、输入/输出边界和少量条件性强调。不要把所有规则都包进 XML，也不要把所有 XML tag 都写成 `<important if="...">`。
...
技术结论是：heading 在 XML 内外 Claude 都能处理。工程结论是：可维护的全局提示词更适合让 heading 在外、XML 做局部语义标注。

## 资料来源

调研日期：2026-04-30。本文依据 Anthropic prompt engineering 文档、Claude Code memory 文档、HumanLayer 博客，以及从 `@anthropic-ai/claude-code` npm 包 source map 还原的 Claude Code TypeScript 源码观察；源码版本为 `2.1.88`。
...
- `claude-code-sourcemap/restored-src/src/constants/prompts.ts:186-197`：生成 `# System`，下面是 bullets。
- `claude-code-sourcemap/restored-src/src/constants/prompts.ts:199-253`：生成 `# Doing tasks`，下面是 bullets。
- `claude-code-sourcemap/restored-src/src/tools/TodoWriteTool/prompt.ts:27-45`：`## Examples of When to Use the Todo List` 下面使用 `<example>` 包示例。
- `claude-code-sourcemap/restored-src/src/services/compact/prompt.ts:79-129`：使用 `<example>`、`<analysis>`、`<summary>` 包结构化示例。

同时，源码也证明 XML tag 内可以包含 Markdown heading：

- `claude-code-sourcemap/restored-src/src/utils/api.ts:463-469`：Claude Code 把动态上下文包进 `<system-reminder>`，内部拼接 `# ${key}` 形式的 Markdown heading，并附带 “may or may not be relevant” 提醒。
...
- 本地 source map: `claude-code-sourcemap/restored-src/src/constants/prompts.ts`
- 本地 source map: `claude-code-sourcemap/restored-src/src/utils/api.ts`
- 本地 source map: `claude-code-sourcemap/restored-src/src/tools/TodoWriteTool/prompt.ts`
- 本地 source map: `claude-code-sourcemap/restored-src/src/services/compact/prompt.ts`
