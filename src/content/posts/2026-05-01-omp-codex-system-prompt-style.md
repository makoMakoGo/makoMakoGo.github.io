...
这篇记录对比的是 Oh My Pi 与 Codex CLI 内置系统提示词的风格差异。重点不在“谁更先进”，而在两套 agent harness 如何通过提示词把产品取向、工具纪律和完成边界固化下来。

## TL;DR

- Codex CLI 的内置提示词更像 Markdown-native 的工程协作说明，强调自然、可读、可审阅，以及本地 coding teammate 的产品体验。
- Oh My Pi 的内置提示词更像 XML-heavy 的执行契约，用 RFC 2119 关键字、结构标签、工具边界和完成边界压低歧义。
- 两者都不是单层 prompt：Codex 通过 Responses API item、AGENTS.md、developer instructions、tools schema 和 skills 分层组装；OMP 则把更多动态规则显式渲染进系统模板。
- Codex 更偏 Unix / patch / plan 的通用开发者工作流；OMP 更偏专用工具路由、并行检索、subagent 分工和高可靠交付纪律。
- 最终差异可以概括为：Codex 把系统提示词设计成“可协作的操作说明”；OMP 把系统提示词设计成“可执行的工程约束”。
...
- 本地 Codex prompt 源码副本：`codex-rs/core/gpt-5.2-codex_prompt.md`
- 本地 Codex model catalog 源码副本：`codex-rs/models-manager/models.json`
- 本地 OMP system prompt 源码副本：`packages/coding-agent/src/prompts/system/system-prompt.md`
